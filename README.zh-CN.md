...
服务会自动在默认浏览器打开 `http://127.0.0.1:8765`。使用 `--no-browser` 可禁止自动打开，使用 `--port PORT` 可选择其他本机端口；服务不会监听外部网络地址。

GUI 只操作仓库自己的目录：源视频放入 `input_videos/`，模型权重或 OpenVINO 导出放入 `models/weights/`，所有结果保存在 `output_videos/`。界面同一时间只运行一个任务，避免重复加载模型争抢内存或显存；它支持实时日志、停止任务、覆盖确认，并会在重启后重新扫描已有结果。
