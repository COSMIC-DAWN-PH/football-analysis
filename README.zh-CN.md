...
上游旧版 `object_tracks.json` 数组属于历史格式。新版采用 JSON Lines，应逐行解析，不能把整个文件当成一个 JSON 数组。使用同一轨迹目录启动新任务时，程序会先删除该目录中上次生成的三个 JSONL 文件。
...
仓库提供了一个只在本机运行的浏览器界面，用于环境检查、视频分析和战术总结。先在同一个虚拟环境中安装 GUI 依赖：
...
Windows 也可以直接双击仓库根目录的 `start_gui.cmd` 一键启动。服务会自动在默认浏览器打开 `http://127.0.0.1:8765`。不要直接双击 `gui/index.html`，因为环境检查和视频上传都需要本地 Python 服务。使用 `--no-browser` 可禁止自动打开，使用 `--port PORT` 可选择其他本机端口；服务不会监听外部网络地址。

GUI 只操作仓库自己的目录：源视频可以在页面中点击选择或拖放上传，也可以手动放入 `input_videos/`；模型权重或 OpenVINO 导出放入 `models/weights/`，所有结果保存在 `output_videos/`。分析表单已经接入独立足球模型和动态镜头标定所需的实测球场长宽，并会把相同尺寸带入战术总结任务。上传支持 MP4、MOV、AVI、MKV、M4V 和 WebM，单文件最大 20 GiB，同名文件会自动编号且不会覆盖原文件。界面同一时间只运行一个分析任务，避免重复加载模型争抢内存或显存；它支持实时日志、停止任务、覆盖确认，并会在重启后重新扫描已有结果。

CPU 上的视频分析可能明显慢于源视频时长。只关闭浏览器页面不会停止任务，只要本地服务仍在运行，重新打开页面即可继续查看。
